...
noCarta *monte=NULL;
noCarta *pilhaDeBaixo[7]={NULL,NULL,NULL,NULL,NULL,NULL,NULL};
noCarta *pilhaDeCima[4]={NULL,NULL,NULL,NULL};
noCarta *atual=NULL;

int linhaEscondida[7];

void criarMonte() //Aloca o baralho a ser usado no jogo
{
	int c1,c2;
	noCarta *n;
...
noCarta *removeMonte(int posicao) //Remove uma carta do monte
{
	noCarta *atual=monte;
...
void imprimeMonte() //Imprime todas as cartas do monte, função de debug
{
	noCarta *atual=monte;
...
void embaralharMonte() //Embaralha todas as cartas do monte
{
	noCarta *n,*novomonte;
	int c;
...
void distribuiCartas() //Cria a mesa do jogo, distribui as cartas pelas colunas de baixo
{
	noCarta *atual;
	int c1,c2;
...
void imprimeCartas() //Imprime a mesa do jogo
{
	...
	while(baixo[0]!=NULL || baixo[1]!=NULL || baixo[2]!=NULL || baixo[3]!=NULL || baixo[4]!=NULL || baixo[5]!=NULL || baixo[6]!=NULL)
	{
		c1++;
		printf("%2d  ",c1);
		for(c2=0;c2<7;c2++)
...
int podeMover(noCarta *rem,noCarta *add) //Verifica se uma operação de mover cartas é válida
{
...
int moveBaixoBaixo(int i,int j,int j2)
{
	...
	if(add==NULL)
	{
		...
		if((add->naipe=='c' && rem->naipe=='o') || (add->naipe=='o' && rem->naipe=='c') || (add->naipe=='p' && rem->naipe=='e') || (add->naipe=='e' && rem->naipe=='p') || (add->naipe=='c' && rem->naipe=='c') || (add->naipe=='e' && rem->naipe=='e') || (add->naipe=='o' && rem->naipe=='o') || (add->naipe=='p' && rem->naipe=='p'))
			return 0;
...
{
	noCarta *n1=pilhaDeBaixo[j], *n_ant=monte,*n2;
	int c;
	...
	printf("\n[%d%c]\n",n1->valor,n1->naipe);
		...
		if((n1->naipe=='c' && atual->naipe=='o') || (n1->naipe=='o' && atual->naipe=='c') || (n1->naipe=='p' && atual->naipe=='e') || (n1->naipe=='e' && atual->naipe=='p') || (n1->naipe=='c' && atual->naipe=='c') || (n1->naipe=='e' && atual->naipe=='e') || (n1->naipe=='o' && atual->naipe=='o') || (n1->naipe=='p' && atual->naipe=='p'))
			return 0;
	}
...
{
	noCarta *n1=pilhaDeBaixo[j],*n2,*n1_ant=NULL;
	int c;
...
int moveMonteCima()
{
	noCarta *n1=atual,*n2,*n1_ant=monte;
	int c;
...
int moveCimaBaixo(char c,int j)
{
	...
	if(n2==NULL)
	{
		...
		if((n1->naipe=='c' && n2->naipe=='o') || (n1->naipe=='o' && n2->naipe=='c') || (n1->naipe=='p' && n2->naipe=='e') || (n1->naipe=='e' && n2->naipe=='p') || (n1->naipe=='c' && n2->naipe=='c') || (n1->naipe=='e' && n2->naipe=='e') || (n1->naipe=='o' && n2->naipe=='o') || (n1->naipe=='p' && n2->naipe=='p'))
			return 0;
